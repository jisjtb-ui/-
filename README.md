...
- `16_クラウドへ渡す.bat` … 予約をクラウドへ登録する。**これを押したらPCの電源を切ってよい**
- `17_クラウドの状況.bat` … クラウドの予約状況を見て、投稿結果をPCへ取り込む
...
最終ページで、4つのアクションにそれぞれ結果を割り当てます。
見る人は「欲しい結果」のアクションを押すため、どこに良い結果を置くかが
そのまま誘導になります。
...
結果は4段階（weak / mid / good / best）で、アクションごとの確率で配ります。
プロフィールは開かれても得るものが小さいため、当たり枠にしません。

| アクション | 中心となる強さ | 最も弱い結果になる確率 |
| --- | --- | --- |
| プロフィール | 最も弱い | 75% |
| いいね | 中程度 | 12% |
| フォロー | かなり良い | 5% |
...
毎回同じ序列だとパターンを読まれるため、確率で配って順位が入れ替わるようにしています。
弱い結果は人格否定ではなく「まだ友達かも」「今は様子見」程度に留めています。

重みも文言も **`data/cta_actions.json`** にあり、コードには固定していません。

```
python generate.py --list-action-cta          # 重みを確認
python generate.py --action-set flat          # 別のセットで生成
python generate.py --no-action-cta            # 従来の保存→共有→コメントに戻す
python tools/check_layout.py output           # 安全域からはみ出していないか確認
```
