...
画像の更新日時を**1分ずつずらして**あるので、
名前順で並べても日時順で並べても、同じ並び（01→10）になります。

それでも順番が崩れる場合は、選び方を変えてみてください。

| 方法 | |
| --- | --- |
| **1枚ずつ追加する** | いちばん確実。10回クリックしますが崩れません |
| まとめて選ぶ | `01` をクリック → `10` を **Shift+クリック**（`Ctrl+A` は避ける） |
| 並べ替えを名前順に | エクスプローラーの表示を「名前」順にしてから選ぶ |
...
**文字を打つのは2か所だけです。**
Categoryを作るときと、SubCategoryを自分で足すときだけ。
それ以外は選ぶ・押す・ON/OFFだけで進みます。

### Category タブ

```
Category                SubCategory（チェックを外すと抽選から除く）
┌──────────┐          ┌──────────────────────────────┐
│ 心理テスト  │          │ 一人の時間        入れる   4.8% │
│           │          │ ホテル・泊まり      入れる   4.8% │
└──────────┘          └──────────────────────────────┘
[心理テスト][＋ 作成]     [＋ SubCategory] [抽選に入れる／外す]

このCategoryのSNSアカウント
  threads     接続済み @xxxx      [接続]
  instagram   未接続              [接続]
  tiktok      未接続              [接続]
  pinterest   未接続              [接続]
```

- Categoryを1つ作ると、SubCategoryは `data/tests` の見出しから
  **自動で登録されます**（入力は要りません）
- `[接続]` を押した時点で、どのCategoryに繋ぐかは決まっています。
  認証が終わったあとに「どのCategoryですか？」と聞き直しません
- 既にThreads / Instagramを接続済みなら、初回起動時に
  そのままCategoryへ**引き継がれます**（接続のやり直しは不要）
- Categoryごとに別のアカウントを繋げます。トークンは
  `.tokens/cat<番号>/` に分かれて保存されます

### 生成 タブ

```
Category      [ 心理テスト          ▼ ]
SubCategory   [ おまかせ（成績に応じて自動で選ぶ） ▼ ]
生成数        [ 10 ]
投稿先        threads @xxxx / instagram @xxxx

[生成]  [接続済み媒体へ予約]  [投稿先を再確認]
```

- SubCategoryの選択肢は、**選んだCategoryのものだけ**が出ます
- 「おまかせ」を選ぶと、成績に応じた割合で1件ずつ選ばれます
- `[接続済み媒体へ予約]` は、そのCategoryに繋いだ媒体を自動で使います。
  アカウント名やIDを打ち込む場面はありません
- 内部では `category_id` / `sub_category_id` だけを受け渡しています

### 成績 タブ

```
Category [ 心理テスト ▼ ] [更新] ☑自動最適化 [いま集計して割合を更新]

配信できた投稿 336件（instagram 168件 / threads 168件）
反応データ: 測定済み 1680件／取得失敗 2件／取得不可 3件  最終取得 09-24 03:10
評価: 24h時点の views / 直近20件の中央値（媒体内の相対値で比較）
実効の下限・上限: 2.38% 〜 40.0%
  ※ 下限 5.0% × 21件 = 105% は100%を超えるため 2.38% に下げました

SubCategory        割合   前回差   中央値  件数  状態
言えていない本音     20.1%   +2.4    3,983   20
一人の時間          2.4%   -0.9      613   20  サンプル不足 3/5

[選んだ行を固定／解除]  [初期値（均等）へ戻す]  [詳しい成績を開く]
```

行を選ぶと、その割合が変わった理由が下に出ます。

## 1枚目のフックを試す（A/B/Cテスト）

1枚目で止まるか進むかで数字は大きく変わります。**1枚目だけ**を差し替えて、
どれが効くか比べられるようにしてあります。
...
6枚目  Q3 問題      7枚目  Q3 答え
8枚目  Q4 問題      9枚目  Q4 答え
10枚目 Q5 問題      11枚目 Q5 答え
```

**問題・回答・デザイン・投稿時刻は3つとも同じにします。**
同時にいくつも変えると、数字が動いた理由が分からなくなるためです。

### 用意してある3種類

| | 種類 | 文言 |
| --- | --- | --- |
| A | 好奇心型 | 本音が出る恋愛心理 ／ 恋人とやってみて🌸 |
| B | カップル参加型 | 恋人と答え合わせしてみて ／ 何個同じになる？🌸 |
| C | 挑発型 | これ一致しないカップル ／ 結構いる。 |

文言は `data/hooks.json` にあります。**D・E… を足すときは、そこへ1件
加えるだけです。**コードを触る必要はありません。

### 画面から選ぶ

`14_画面をひらく.bat` →「Category」タブ →「1枚目フック」
...
単純な A→B→C の繰り返しではなく、1周ごとに1つずらしています。
1日に3回投稿する運用でも「Aはいつも朝」のように**時間帯が固定されない**
ようにするためです。9投稿で、どのフックもどの時間帯に3回ずつ出ます。

### Instagramに出すときの注意

**Instagramのカルーセルは10枚が上限です（公式仕様）。11枚は投稿できません。**

| 問題数 | 枚数 | Threads | Instagram | TikTok |
| --- | --- | --- | --- | --- |
| 5問 | 11枚 | ○ | **×** | ○ |
| 4問 | 9枚 | ○ | ○ | ○ |

Instagramにも出す場合は `.env` で `TESTS_PER_POST=4` にしてください。
11枚のまま生成すると、生成時に注意が表示されます。

### 成績を見る

「成績」タブの下に「1枚目フックの比較」が出ます。

```
フック            投稿  表示（中央値）  保存率   共有率  コメント率
A 好奇心型         12        4,944    0.99%   0.39%    0.19%
B カップル参加型     12        5,186    1.19%   0.47%    0.19%
C 挑発型           12        1,606    4.46%   1.76%    0.18%
```

コマンドでも見られます。

```bat
python autopost.py hooks list      使えるフックと文言
python autopost.py hooks compare   フック別の成績
```

**総再生数だけで決めないでください。** 上の例では、Cは表示がAの3分の1ですが
保存率は4倍以上あります。表示回数は投稿のタイミングや運にも左右されるため、
率（保存数 ÷ 表示数）の方がフックの良し悪しを表します。
...
表示数は `views` を使い、取れない場合は `impressions` → `reach` の順で
**実際に取れた数字**に切り替えます。平均視聴時間と完了率はReel専用の指標で、
カルーセル投稿では取得できないため空欄になります（推測値は入れません）。

### いつ判断するか

**各フック10件そろうまでは「まだ判断しないでください」と表示されます。**
数件で決めると、たまたま伸びた1本に引きずられます。

いまは3種類を**均等に**出すことを優先しています。成績の良いフックの割合を
自動で増やす仕組みは、データが集まってから足せる構造にしてありますが、
まだ動いていません。

## 成績の良いSubCategoryを自動で増やす

反応の良かったSubCategoryを、次回から少しずつ多く作ります。
画面の「成績」タブ、または `13_成績を見る.bat` で、いまの割合と
「なぜ変わったのか」が読めます。

**変えるのはSubCategoryの出現割合だけです。**
...
反応データを取る     →  5_反応データを集める.bat（1h / 6h / 24h / 72h / 7d）
SubCategory別に集計  →  媒体内の相対値に直してから、直近20件の「中央値」
weightを更新        →  全体の中央値と比べて、上下に最大3%動かす
次回の抽選に反映     →  「おまかせ」で生成したとき（自動補充もこれ）
```

媒体ごとに閲覧の規模が違うため、そのまま混ぜると規模の大きい媒体だけで
決まってしまいます。**まず媒体内での相対値に直してから**まとめます。

同じ測定データで何度更新しても、2回目以降は何も変わりません
（使った測定データの指紋を履歴に残して見分けています）。
...
### 0になるSubCategoryを作らない

成績が悪くても、weightには**下限**があります（既定5%）。
下限に達したものは、そこで下げ止まります。たまたま数字が
悪かっただけのSubCategoryを二度と作らなくなる、という事故を防ぐためです。

数が多いと「21件 × 最低5% = 105%」のように成立しなくなるため、
その場合は下限を自動で下げます（21件なら約2.38%）。
**実効の下限・上限と、下げた理由は成績画面に出ます。**

### 遅れて測ったデータの扱い

`24h` 時点のデータは、投稿から24時間後〜30時間後（許容6時間）の間に
測ったものだけを使います。1週間放置した投稿の「いまの数字」を
24h時点の数字として混ぜると、比較が成り立たなくなるからです。

許容を過ぎてから測ったものは**遅延として記録し、通常の評価には
混ぜません**（記録は残るので、あとから見ることはできます）。
実測していない過去時点の数字は作りません。
...
ふだんは画面で足りますが、コマンドからも操作できます。
`category list` でID（画面と同じもの）が確認できます。

```bat
rem Category と SubCategory の一覧・ID・接続状況
python autopost.py category list
...
rem 自動最適化のON/OFF（画面のチェックと同じ）
python autopost.py weights auto off

rem 特定のSubCategoryを自分で決めて、自動更新の対象から外す
python autopost.py weights set 11 25
python autopost.py weights lock 11
python autopost.py weights unlock 11

rem 均等（初期値）へ戻す
python autopost.py weights reset
```

`lock` したものは、自動更新でも、SubCategoryを追加したときの
再計算でも動きません。

自動最適化をOFFにしても、**設定した割合での抽選は続きます**。
止まるのは「割合が自動で動くこと」だけです。
...
| 指標 | Threads | Instagram（カルーセル） | TikTok（写真・下書き転送） | 備考 |
| --- | --- | --- | --- | --- |
| views | ○ | ○ | △ | 既定の評価指標 |
| reach | × | ○ | × | 見た人数（重複なし） |
...
| 視聴完了率 | × | × | × | 同上 |
| フォロワー増加（投稿単位） | × | × | × | どのAPIも投稿ごとには返しません |

TikTokの △ について。Display API の `/v2/video/query/`（`video.list` スコープ）で
`view_count` / `like_count` / `comment_count` / `share_count` が取れますが、

- 返るのは**動画**の情報で、写真投稿が対象かは公式ドキュメントに記載がありません
- 本システムは**下書き転送**のため、手元に残るのは `publish_id` だけです。
  これは動画IDではないので、この問い合わせには使えません

そのため通常は **取得不可** として記録します（取得失敗とは別に数えます）。
公開済みの動画IDが手元にある場合だけ問い合わせます。

`views` が取れなかった投稿は `impressions` → `reach` の順に、
**実際に取れた数字**へ切り替えます（推測はしません）。

取れなかったときは空欄のまま残し、**測定済みにしません**。
「0件だった」と「取れなかった」は別に扱います。成績画面では
測定済み／取得失敗／取得不可／レート制限で保留を分けて数えます。

権限不足・使えない指標・レート制限・通信失敗は区別して扱います。
レート制限は待ってから再試行し、それでも駄目なら次回に回します。
**取得に失敗しても、投稿の処理は止まりません。**
